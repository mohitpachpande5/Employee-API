...
The `java.nio` package has the following subpackages:  

1. `java.nio.file` → Handles file and directory operations, including paths, file attributes, and filesystem access.  
2. `java.nio.charset` → Supports character encoding and decoding, providing various charsets and their operations.  
3. `java.nio.channels` → Provides channel-based I/O, enabling asynchronous and buffer-oriented data transfers.  
4. `java.nio.channels.spi` → Contains service-provider classes for implementing custom channels and selectors.  
5. `java.nio.file.attribute` → Defines classes and interfaces for accessing and modifying file attributes.  
6. `java.nio.file.spi` → Provides service-provider interfaces for custom filesystem implementations.  
...
│
├── ByteOrder (class)
├── MappedByteBuffer (abstract class)
├── ReadOnlyBufferException (class)  <-- (RuntimeException)
├── InvalidMarkException (class)  <-- (RuntimeException)
│
└── java.nio.file
    │
    ├── Files (class)
    ...
    ├── FileStore (abstract class)
    ├── FileSystems (class)
    ├── FileSystem (abstract class)
    ├── FileVisitOption (enum)
    ├── FileVisitor<T> (interface)
    ├── SimpleFileVisitor<T> (class)
    ...
    │
    ├── Exception Classes:
    │   ├── DirectoryIteratorException (class)  <-- (RuntimeException)
    │   ├── FileAlreadyExistsException (class)  <-- (IOException)
    │   ├── FileSystemAlreadyExistsException (class)  <-- (RuntimeException)
    │   ├── FileSystemException (class)  <-- (IOException)
    │   ├── FileSystemLoopException (class)  <-- (FileSystemException)
    │   ├── FileSystemNotFoundException (class)  <-- (RuntimeException)
    │   ├── InvalidPathException (class)  <-- (IllegalArgumentException)
    │   ├── NoSuchFileException (class)  <-- (FileSystemException)
    │   ├── NotDirectoryException (class)  <-- (FileSystemException)
    │   ├── DirectoryNotEmptyException (class)  <-- (FileSystemException)
    │   ├── AccessDeniedException (class)  <-- (FileSystemException)
    │   ├── ProviderNotFoundException (class)  <-- (RuntimeException)
    │   ├── AtomicMoveNotSupportedException (class)  <-- (FileSystemException)
    │
    └── java.nio.charset
        │
        ├── Charset (class)
        ...
        ├── CoderResult (class)
        ├── CodingErrorAction (enum)
        ├── StandardCharsets (class)
        │
        ├── Exception Classes:
        │   ├── UnsupportedCharsetException (class)  <-- (IllegalArgumentException)
        │   ├── IllegalCharsetNameException (class)  <-- (IllegalArgumentException)
        │   ├── CharacterCodingException (class)  <-- (IOException)
        │   ├── MalformedInputException (class)  <-- (CharacterCodingException)
        │   ├── UnmappableCharacterException (class)  <-- (CharacterCodingException)
        │
        └── java.nio.channels
            │
            ├── Channel (interface)
            ...
            ├── SelectionKey (abstract class)
            ├── Selector (class)
            ├── FileLock (class)
            │
            ├── Exception Classes:
            │   ├── ClosedChannelException (class)  <-- (IOException)
            │   ├── NonWritableChannelException (class)  <-- (IllegalStateException)
            │   ├── NonReadableChannelException (class)  <-- (IllegalStateException)
            │   ├── UnresolvedAddressException (class)  <-- (IllegalStateException)
            │   ├── UnavailableChannelException (class)  <-- (IllegalStateException)
            │   ├── AlreadyConnectedException (class)  <-- (IllegalStateException)
            │   ├── ConnectionPendingException (class)  <-- (IllegalStateException)
            │   ├── NotYetBoundException (class)  <-- (IllegalStateException)
            │   ├── NoConnectionPendingException (class)  <-- (IllegalStateException)
            │   ├── AsynchronousCloseException (class)  <-- (ClosedChannelException)
            │   ├── FileLockInterruptionException (class)  <-- (IOException)
